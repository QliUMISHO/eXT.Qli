...
# WebRTC shared state is owned by the asyncio event loop.
webrtc_state_lock: Optional[asyncio.Lock] = None
viewer_offer_state: Dict[str, str] = {}
...
def handle_task(task_data: Dict[str, Any], task_id: Optional[str]) -> str:
    """Handle dashboard tasks received through the WebRTC data channel.

    This refactor intentionally keeps only support/diagnostic tasks. High-risk task
    families such as keylogging, persistence, privilege escalation, arbitrary shell
    execution, raw TCP listeners, and file exfiltration are not executed here.
    """
    task_name = str(task_data.get("task", "")).strip().lower()
    ...
    if task_name in {"screenshot", "thumbnail"}:
        result = screenshot()
        if task_name == "thumbnail" and result.startswith("SCREENSHOT:"):
            return "THUMBNAIL:" + result.split(":", 1)[1]
        return result

    if task_name == "webcam":
        return "Unsupported task: webcam capture is disabled in this safe refactor."
    ...
    if task_name in {
        ...
        return f"Unsupported task: {task_name} is disabled in this safe refactor."
...
async def poll_for_offers() -> None:
    """Poll signaling inside the WebRTC asyncio loop; no worker thread is used."""
    while not STOP_EVENT.is_set():
        try:
            resp = await asyncio.to_thread(
                http_get_json,
                f"{SIGNALING_URL}?action=poll_offer&agent_uuid={AGENT_UUID}",
            )
            if resp.get("has_offer"):
                offer_sdp = resp.get("offer_sdp")
                viewer_id = resp.get("viewer_id")
                if offer_sdp and viewer_id:
                    lock = webrtc_state_lock
                    if lock is None:
                        raise RuntimeError("WebRTC state lock is not initialized")
                    async with lock:
                        if viewer_offer_state.get(viewer_id) == "processing":
                            log(f"Ignoring duplicate offer for {viewer_id} (already processing)")
                            await asyncio.sleep(SIGNALING_POLL_INTERVAL_SECONDS)
                            continue
                        viewer_offer_state[viewer_id] = "processing"
                    log(f"Received offer from {viewer_id}")
                    asyncio.create_task(establish_webrtc(offer_sdp, viewer_id))
        except Exception as exc:
            log(f"Polling offers error: {exc}")
        await asyncio.sleep(SIGNALING_POLL_INTERVAL_SECONDS)


async def cleanup_webrtc_viewer(viewer_id: str, close_peer: bool = True) -> None:
    lock = webrtc_state_lock
    if lock is None:
        return
    async with lock:
        pc = webrtc_peers.pop(viewer_id, None)
        webrtc_data_channels.pop(viewer_id, None)
        track = screen_tracks.pop(viewer_id, None)
        viewer_offer_state.pop(viewer_id, None)
    ...
    if close_peer and pc is not None:
        try:
            ...
            log(f"Error closing peer for {viewer_id}: {exc}")

async def mark_offer_done(viewer_id: str) -> None:
    lock = webrtc_state_lock
    if lock is None:
        return
    async with lock:
...
async def establish_webrtc(offer_sdp: str, viewer_id: str):
    ...
    if not WEBRTC_AVAILABLE:
        ...
        await mark_offer_done(viewer_id)
        return

    await cleanup_webrtc_viewer(viewer_id, close_peer=True)
    ...
    try:
        ...
        async def on_connectionstatechange():
            ...
            if pc.connectionState == "connected":
                ...
                await cleanup_webrtc_viewer(viewer_id, close_peer=False)
                ...
                if lock is not None:
                    async with lock:
                        track = screen_tracks.get(viewer_id)
                        if track:
                            track.paused = True
                await mark_offer_done(viewer_id)
        ...
        try:
            await asyncio.to_thread(send_signal, "submit_answer", {
                "viewer_id":  viewer_id,
...
# Main agent loop
# =========================
async def agent_async_main() -> None:
    global webrtc_loop, webrtc_state_lock

    webrtc_loop = asyncio.get_running_loop()
    webrtc_state_lock = asyncio.Lock()

    heartbeat_thread = threading.Thread(target=heartbeat_loop, name="heartbeat-loop", daemon=True)
    heartbeat_thread.start()

    poll_task = asyncio.create_task(poll_for_offers(), name="signaling-poller")
    ...
    try:
        ...
        if lock is not None:
            async with lock:
                peers = list(webrtc_peers.items())
                webrtc_peers.clear()
                webrtc_data_channels.clear()
                tracks = list(screen_tracks.values())
                screen_tracks.clear()
                viewer_offer_state.clear()
            for track in tracks:
                try:
                    track.sct.close()
                except Exception:
        ...
        for viewer_id, pc in peers:
            try:
                await pc.close()
            except Exception as exc:
                log(f"Error closing peer {viewer_id}: {exc}")


def main() -> int:
    log("Starting agent (WebRTC + MySQL HTTP signaling) - Multi-viewer support")
    log(f"Agent UUID: {AGENT_UUID}")
    log(f"State file: {STATE_FILE}")
    log("HTTP task server disabled; tasks are accepted only through WebRTC data channels.")

    try:
        asyncio.run(agent_async_main())
